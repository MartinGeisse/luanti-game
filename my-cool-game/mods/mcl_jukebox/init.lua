...
function mcl_jukebox.register_record_definition(def)
	...
	local entryname = "Music Disc"
	local longdesc = "A music disc holds a single music track which can be used in a jukebox to play music."
	local usagehelp = "Place a music disc into an empty jukebox to play the music. Use the jukebox again to retrieve the music disc. The music can only be heard by you, not by other players."
	core.register_craftitem(":"..itemstring, {
		description = "Music Disc",
		_tt_help = def.author .. "-" .. def.title,
...
local function now_playing(player, name)
	...
	local text = "Now playing: " .. mcl_jukebox.registered_records[name].author .. "—" .. mcl_jukebox.registered_records[name].title
...
core.register_craft({
	...
	recipe = {
		...
		{"mts_default:rock", "mts_default:pebble", "mts_default:rock"},
		{"mts_default:rock", "mts_default:rock", "mts_default:rock"},
	}
})
core.register_craft({
...
core.register_node("mcl_jukebox:jukebox", {
	description = "Jukebox",
	_tt_help = "Uses music discs to play music",
